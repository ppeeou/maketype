...
describe('Number', function () {
  ...
  it("toNumber", function (done) {
    ...
    assert.equal(makeType.toNumber(function () { }), 0);
...
describe('String', function () {
  ...
  it("toString", function (done) {
    ...
    assert.equal(makeType.toString('5'), '5');
    assert.equal(makeType.toString(function () { }), '[object Function]');
...
describe('Function', function () {
  it("isFunction", function (done) {
    ...
    assert.equal(makeType.isFunction(function () { }), true);
...
describe('Array', function () {
  ...
  it("toArray", function (done) {
    ...
    assert.deepEqual(makeType.toArray({ a: 1, b: 2, c: 3, d: 4, e: 5 }), [1, 2, 3, 4, 5]);
    done();
  });
});
...
describe('Iterable', function () {
  it("isGenerator", function (done) {
    ...
    assert.equal(makeType.isGenerator(function () { }), false);
  ...
  it("isIterable", function (done) {
    ...
    assert.equal(makeType.isIterator(function () { }), false);
  ...
  it("toIterator", function (done) {
    ...
    iter = makeType.toIterator((function* () {
      yield 1;
      yield 2;
      yield 3;
    })());
...
describe('Promise', function () {
  ...
  it("toPromise(common)", function (done) {
    const add10 = (a, cb) => cb(null, a + 10);
    const add10P = makeType.toPromise(add10);

    add10P(5)
      .then(res => assert.equal(res, 15))
      .then(done);
  });
  ...
  it("toPromise wrapper (fail case)", function (done) {
    delayP(10, false)
      .catch(error => assert.equal(error, 'error'))
      .then(done);
  });
});